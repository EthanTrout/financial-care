    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if session.get("user_access") not in allowed_roles:
                # Redirect to a general page or an error page
...
def edit_service(service_id):
    ...
    staff_members = Staff.query.join(staff_service).join(
        Service).filter(Service.id == service_id).all()
    staff_info = [{"id": staff.id, "name": staff.name}
...
def service_users():
    if session["user_access"] in ["manager", "it"]:
        service_users = list(
            ServiceUser.query.order_by(ServiceUser.name).all())
        ...
        service_users = ServiceUser.query.filter(
            ServiceUser.service_id.in_(service_ids)).all()
        return render_template("service_users.html", service_users=service_users)
...
def open_wallet(service_user_id):
    ...
    if last_wallet_entry.is_cash_removed == True:
        ...
        if subsequent_entries:
            cash_spent_values = [
                entry.money_spent for entry in subsequent_entries]
    ...
    if request.method == "POST":
        ...
        wallet_entry = WalletEntry(
            ...
            cash_amount=Decimal(last_wallet_entry.cash_amount) -
            Decimal(request.form.get("cash_out")),
    ...
    if request.method == "POST":
        ...
        if outstanding_money >= money_spent:
            ...
            wallet_entry = WalletEntry(
                ...
                money_spent_description=request.form.get(
            ...
            # Update receipts
            receipt = [receipt_number, request.form.get(
                "money_spent_description"), float(money_spent)]
...
def close_wallet_banking(service_user_id, enter_seal):
    ...
    if request.method == "POST":
        ...
        wallet_entry = WalletEntry(
            ...
            cash_amount=last_wallet_entry.cash_amount,
            bank_amount=last_wallet_entry.bank_amount -
            Decimal(request.form.get("bank_out")),
            cash_out=0,
            cash_in=0,
        ...
        db.session.commit()
        receipt = [receipt_number, request.form.get(
            "money_spent_description"), float(request.form.get("bank_out"))]
        all_receipts.append(receipt)
...
def view_wallet(service_user_id):
    wallet_entries = WalletEntry.query.filter_by(
        service_user_id=service_user_id).order_by(WalletEntry.id.desc()).all()
    ...
    if request.method == "POST":
        if bank_in == "True":
            wallet_entry = WalletEntry(
                ...
                cash_amount=last_wallet_entry.cash_amount,
                bank_amount=last_wallet_entry.bank_amount +
                Decimal(request.form.get("bank_in")),
                cash_out=0,
                cash_in=0,
                bank_card_removed=bool(False),
                money_spent=0,
                money_spent_description="Reconsiliation: " +
                request.form.get("money_spent_description"),
                bank_out=0,
