...
class Staff(db.Model):
    ...
    services = db.relationship(
        "Service", secondary=staff_service, backref="staff")

    def __repr__(self):
        return f"<staff: {self.name}>"

    def set_password(self, plain_text_password):
        self.password = bcrypt.hashpw(plain_text_password.encode(
...
class ServiceUser(db.Model):
    ...
    wallet_entries = db.relationship(
        "WalletEntry", backref="service_user", cascade="all, delete", lazy=True)


class WalletEntry(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    service_user_id = db.Column(db.Integer, db.ForeignKey(
        "service_user.id"), nullable=False)
